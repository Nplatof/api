...
@app.route('/v2/<string:Etudiants>/', methods=['GET'])
def getEtudiants(Etudiants):
    if Etudiants.lower() != 'etudiants':
        return jsonify({'erreur': 'URL invalide'}), 404  # Custom error message for wrong URL
    ...
    try:
        ...
        return jsonify({'erreur': 'Une erreur est survenue'}), 500


# Fetch a specific student (GET - retrieval still uses GET)
@app.route('/v2/etudiant/<int:id>', methods=['GET'])
def getEtudiant(id):
...
def createEtudiant():
    ...
    if not (nom.isalpha):
        return TypeError("Impossible de créer l'étudiant car le nom est invalide.")
    ...
    try: 
        ...
        return jsonify({"message": "Impossible de créer l'etudiant"}), 400



# Delete student
@app.route('/v2/etudiant/<int:id>', methods=['DELETE'])
...
# Modify student
@app.route('/v2/etudiant/<int:id>', methods=['PUT'])
